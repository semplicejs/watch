...
function execFromPackage() {
  ...
  try {
    // note: this isn't sempliceWatch's package, it's the user's cwd package
 ...
 *
 * @param  {Object} sempliceWatchOptions
 * @param  {Object} execMap
 * @return {Object} new and updated version of sempliceWatchOptions
 */
function exec(sempliceWatchOptions, execMap) {
  if (!execMap) {
    execMap = {};
  }
  ...
  // should be parsed first, then the user options (via sempliceWatch.json) then
  // finally default down to pot shots at the directory via package.json
  if (!sempliceWatchOptions.exec && !sempliceWatchOptions.script) {
    var found = execFromPackage();
    if (found !== null) {
      if (found.exec) {
        sempliceWatchOptions.exec = found.exec;
      }
      if (!sempliceWatchOptions.script) {
        sempliceWatchOptions.script = found.script;
      }
      if (Array.isArray(sempliceWatchOptions.args) &&
        sempliceWatchOptions.scriptPosition === null) {
        sempliceWatchOptions.scriptPosition = sempliceWatchOptions.args.length;
      }
    }
  }

  var options = utils.clone(sempliceWatchOptions || {});
  ...
  // https://github.com/remy/sempliceWatch/issues/195
  ...
  if (!execDefined && options.exec === 'node' &&
    ...
    // for example, in `sempliceWatch --debug my-script.coffee --my-flag`, debug is an
  ...
  // BIG NOTE: user can't do this: sempliceWatch -e *.js
  ...
  if (({}).toString.apply(sempliceWatchOptions.env) === '[object Object]') {
    options.env = utils.clone(sempliceWatchOptions.env);
  } else if (sempliceWatchOptions.env !== undefined) {
    throw new Error('sempliceWatch env values must be an object: { PORT: 8000 }');
  }
