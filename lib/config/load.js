...
/**
 * Load the sempliceWatch config, first reading the global root/sempliceWatch.json, then
 * the local sempliceWatch.json to the exec and then overwritting using any user
...
function load(settings, options, config, callback) {
  config.loaded = [];
  // first load the root sempliceWatch.json
  loadFile(options, config, utils.home, function (options) {
    ...
    loadFile(options, config, process.cwd(), function (options) {
      ...
      // if we didn't pick up a sempliceWatch.json file & there's no cli ignores
      // then try loading an old style .sempliceWatchignore file
      if (config.loaded.length === 0) {
        // TODO decide whether this is just confusing...
        var legacy = loadLegacyIgnore.bind(null, options, ready);

        // first try .sempliceWatchignore, if that doesn't exist, try sempliceWatch-ignore
        return legacy('.sempliceWatchignore', function () {
          legacy('sempliceWatch-ignore', function (options) {
            ready(options);
          });
        });
      }
...
/**
 * Loads the old style sempliceWatchignore files which are simply a list of patterns
 * in a file to ignore
 *
 * @param  {Object} options    sempliceWatch user options
 * @param  {Function} success
 * @param  {String} filename   ignore file (.sempliceWatchignore or sempliceWatch-ignore)
...
function loadFile(options, config, dir, ready) {
  ...
  var callback = function (settings) {
    // prefer the local sempliceWatch.json and fill in missing items using
    // the global options
  ...
  var filename = options.configFile || path.join(dir, 'sempliceWatch.json');
  fs.readFile(filename, 'utf8', function (err, data) {
    if (err) {
      if (err.code === 'ENOENT') {
        if (!options.configFile && dir !== utils.home) {
          // if no specified local config file and local sempliceWatch.json
...
function loadPackageJSON(config, ready) {
  ...
  utils.log.detail('Looking in package.json for sempliceWatchConfig');
  ...
  return loadFile(packageLoadOptions, config, dir, function (settings) {
    ready(settings.sempliceWatchConfig || {});
  });
}
