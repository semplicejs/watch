...
sempliceWatch is a utility for node, and replaces the use of the executable
node. So the user calls `sempliceWatch foo.js` instead.

sempliceWatch can be run in a number of ways:

`sempliceWatch` - tries to use package.json#main property to run
`sempliceWatch` - if no package, looks for index.js
`sempliceWatch app.js` - runs app.js
`sempliceWatch --arg app.js --apparg` - eats arg1, and runs app.js with apparg
`sempliceWatch --apparg` - as above, but passes apparg to package.json#main (or
  index.js)
`sempliceWatch --debug app.js
 ...
 * sempliceWatch options, the user script and the executable script.
 *
...
function parse(argv) {
  ...
  var script = null;
  var sempliceWatchOptions = { scriptPosition: null };

  var sempliceWatchOpt = sempliceWatchOption.bind(null, sempliceWatchOptions);
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (!script) {
      if (args[i] === '.' || existsSync(args[i])) {
        ...
        sempliceWatchOptions.scriptPosition = i;
        i--;
        continue;
      }
    }
    ...
    if (lookForArgs) {
      ...
      if (args[i] === '--') {
        ...
        // ignore all further sempliceWatch arguments
        lookForArgs = false;
      ...
      if (sempliceWatchOpt(args[i], eat.bind(null, i, args)) !== false) {
        args.splice(i, 1);
        // cycle back one argument, as we just ate this one up
        i--;
      }
  ...
  if (script === null && !sempliceWatchOptions.exec) {
    var found = findAppScript();
    if (found !== null) {
      if (found.exec) {
        sempliceWatchOptions.exec = found.exec;
      }
      script = found.script;
      sempliceWatchOptions.scriptPosition = args.length;
    }
  }

  sempliceWatchOptions.script = script;
  sempliceWatchOptions.args = args;

  return sempliceWatchOptions;
}


/**
 * Given an argument (ie. from process.argv), sets sempliceWatch
 ...
 * @return {Boolean} false if argument was not a sempliceWatch arg
 */
function sempliceWatchOption(options, arg, eatNext) {
  ...
  if (arg === '--cwd') {
    ...
    // go ahead and change directory. This is primarily for sempliceWatch tools like
    // grunt-sempliceWatch - we're doing this early because it will affect where the
...
function findAppScript() {
  // sempliceWatch has been run alone, so try to read the package file
...
/**
 * Given an argument (ie. from sempliceWatchOption()), will parse and return the
