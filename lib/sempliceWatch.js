...
function sempliceWatch(settings) {
  sempliceWatch.reset();

  // allow the cli string as the argument to sempliceWatch, and allow for
  // `node sempliceWatch -V app.js` or just `-V app.js`
  if (typeof settings === 'string') {
    settings = settings.trim();
    if (settings.indexOf('node') !== 0) {
      if (settings.indexOf('sempliceWatch') !== 0) {
        settings = 'sempliceWatch ' + settings;
      }
  ...
  // sempliceWatch tools like grunt-sempliceWatch. This affects where
  // the script is being run from, and will affect where
  // sempliceWatch looks for the sempliceWatch.json files
  if (settings.cwd) {
  ...
  config.load(settings, function (config) {
    ...
    if (config.options.stdin && config.options.restartable) {
      // allow sempliceWatch to restart when the use types 'rs\n'
      ...
      // if 'restartable' is disabled (via a sempliceWatch.json)
    ...
    if (config.options.dump) {
      ...
      utils.log._log('log', 'sempliceWatch: ' + version.pinned);
      ...
      if (config.options.stdout === false) {
        sempliceWatch.on('start', function () {
          sempliceWatch.stdout = bus.stdout;
          sempliceWatch.stderr = bus.stderr;
  ...
  return sempliceWatch;
}

sempliceWatch.restart = function () {
  utils.log.status('restarting child process');
  bus.emit('restart');
  return sempliceWatch;
};

sempliceWatch.addListener = sempliceWatch.on = function (event, handler) {
...
sempliceWatch.once = function (event, handler) {
  if (!eventHandlers[event]) { eventHandlers[event] = []; }
  eventHandlers[event].push(handler);
...
sempliceWatch.emit = function () {
  bus.emit.apply(bus, [].slice.call(arguments));
  return sempliceWatch;
};

sempliceWatch.removeAllListeners = function (event) {
  // unbind only the `sempliceWatch.on` event handlers
...
sempliceWatch.reset = function (done) {
  bus.emit('reset', done);
};

bus.on('reset', function (done) {
  debug('reset');
  sempliceWatch.removeAllListeners();
...
sempliceWatch.config = config;

module.exports = sempliceWatch;
