...
function run(options) {
  ...
  if (runCmd) {
    utils.log.status('starting app'); //+ config.command.string + '`
  }
  ...
  run.kill = function (noRestart, callback) {
    ...
    if (child !== null) {
      ...
      /* Now kill the entire subtree of processes belonging to sempliceWatch */
...
function kill(child, signal, callback) {
  ...
  if (utils.isWindows) {
    ...
    if (hasPS) {
      // we use psTree to kill the full subtree of sempliceWatch, because when
      // spawning processes like `coffee` under the `--debug` flag, it'll spawn
      // it's own child, and that can't be killed by sempliceWatch, so psTree gives us
      //  an array of PIDs that have spawned under sempliceWatch, and we send each the
